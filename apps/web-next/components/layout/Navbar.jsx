...
const Navbar = () => {
    ...
    return (<nav className={cn("fixed top-0 left-0 right-0 z-50 transition-all duration-300 ease-in-out hidden md:block", scrolled
            ...
            : "bg-mn-surface/80 backdrop-blur-md border-b border-white/20 shadow-sm py-4")}>
      <div className="flex justify-between items-center px-16 max-w-full">
        ...
        <Link href="/" className="flex items-center gap-2.5 hover:opacity-80 transition-opacity">
          ...
          <span className="font-headline-md text-xl font-bold text-mn-primary">Mimir <span className="text-surface-tint">Nest</span></span>
        </Link>
        ...
        <div className="flex items-center gap-6">
          {navLinks.slice(1).map((link) => (<Link key={link.name} href={link.path} className={cn("font-body-md text-body-md transition-all duration-300 ease-in-out pb-1", pathname === link.path
                ? "text-mn-primary font-semibold border-b-2 border-mn-primary"
                : "text-on-surface-variant hover:text-mn-primary")}>
              {link.name}
            </Link>))}
        </div>
        ...
        <div className="flex items-center gap-4">
          <button className="text-on-surface-variant hover:text-mn-primary transition-colors p-2 rounded-full hover:bg-surface-container">
            <Search className="w-5 h-5"/>
          </button>
          <Link href="/placement-dsa" className="bg-mn-primary text-on-primary px-6 py-2 rounded-full font-label-caps text-label-caps hover:opacity-90 transition-opacity tracking-widest">
            Get Started
          </Link>
        </div>
      </div>
...
const MobileNav = () => {
    ...
    return (<nav className="md:hidden fixed top-0 w-full z-50 bg-mn-surface/90 backdrop-blur-md px-6 py-4 flex justify-between items-center border-b border-outline-variant/30 shadow-sm">
      <Link href="/" className="flex items-center gap-2.5 hover:opacity-80 transition-opacity">
        <img src="/logo/logo.svg" alt="Mimir Nest" className="h-8 w-auto"/>
        <span className="font-bold text-mn-primary text-xl">Mimir <span className="text-surface-tint">Nest</span></span>
      </Link>
      <button onClick={() => setIsOpen(!isOpen)} className="text-mn-primary p-1 rounded-full hover:bg-surface-container transition-colors">
      ...
      {isOpen && (<div className="absolute top-full left-0 right-0 bg-mn-surface/98 backdrop-blur-2xl border-b border-outline-variant/40 animate-in slide-in-from-top-4 duration-300">
          <div className="px-4 py-6 space-y-1 max-h-[80vh] overflow-y-auto">
            {navLinks.map((link) => (<Link key={link.name} href={link.path} className={cn("flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-medium transition-all duration-200", pathname === link.path
                    ? "bg-primary-container/30 text-mn-primary border border-mn-primary/20"
                    : "text-on-surface-variant hover:text-mn-primary hover:bg-surface-container border border-transparent")}>
